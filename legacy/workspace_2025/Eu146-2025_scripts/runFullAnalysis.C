...
// 分析直方图中的630 keV峰（626-632范围）- 修改为630 keV峰
bool analyzePeak630(TH1D* hist, double& netCounts, double& uncertainty) {
    const double LOW_LIMIT = 626.0;   // 下限
    const double HIGH_LIMIT = 632.0;  // 上限
    
    int binStart = hist->FindBin(LOW_LIMIT);
    int binEnd = hist->FindBin(HIGH_LIMIT);
    
    if (binStart >= binEnd) {
        std::cerr << "Warning: Invalid bin range for 630 keV peak" << std::endl;
        return false;
    }
...
// 分析直方图中的740 keV峰（740-745范围）- 修改为740 keV峰
bool analyzePeak740(TH1D* hist, double& netCounts, double& uncertainty) {
    const double LOW_LIMIT = 740.0;   // 下限
    const double HIGH_LIMIT = 745.0;  // 上限
    
    int binStart = hist->FindBin(LOW_LIMIT);
    int binEnd = hist->FindBin(HIGH_LIMIT);
    
    if (binStart >= binEnd) {
        std::cerr << "Warning: Invalid bin range for 740 keV peak" << std::endl;
        return false;
    }
...
// 从ROOT文件读取直方图并分析峰，返回计数和误差 - 修改为630和740 keV峰
bool analyzeROOTFile(const std::string& rootFilePath, 
                    double& rate630, double& error630,
                    double& rate740, double& error740) {
    ...
    // 分析630 keV峰
    double netCounts630, countsError630;
    bool success630 = analyzePeak630(hist, netCounts630, countsError630);
    
    // 分析740 keV峰
    double netCounts740, countsError740;
    bool success740 = analyzePeak740(hist, netCounts740, countsError740);
    
    rootFile->Close();
    delete rootFile;
    
    // 设置计数和误差
    rate630 = netCounts630;
    error630 = countsError630;
    rate740 = netCounts740;
    error740 = countsError740;
    
    return (success630 && success740);
}
...
void AnalyzeSPEandROOTFiles() {
    ...
    std::cout << "Found " << speFiles.size() << " SPE files from 11.24 to 12.20" << std::endl;
    std::cout << "========================================================================================" << std::endl;
    
    // 输出表头 - 修改为630和740 keV峰
    std::cout << std::left << std::setw(25) << "Filename" 
              << std::setw(12) << "DeadTime(%)"
              << std::setw(15) << "DeadTimeErr(%)"
              << std::setw(20) << "630keV Rate±Err"
              << std::setw(20) << "630keV Corr±Err"
              << std::setw(20) << "740keV Rate±Err"
              << std::setw(20) << "740keV Corr±Err"
              << std::endl;
    ...
    std::cout << "Analyzing 630 keV and 740 keV peaks" << std::endl;
    std::cout << "630 keV peak range: 628-632 keV" << std::endl;
    std::cout << "740 keV peak range: 738-742 keV" << std::endl;
    std::cout << std::endl;
    ...
    for (size_t i = 0; i < speFiles.size(); i++) {
        ...
        // 从ROOT文件分析峰 - 修改为630和740 keV峰
        double counts630, error630, counts740, error740;
        
        bool analysisSuccess = analyzeROOTFile(rootFilePath, counts630, error630, 
                                               counts740, error740);
        
        // 计算原始计数率及其统计误差
        double countRate630 = 0.0, countRateError630 = 0.0;
        double countRate740 = 0.0, countRateError740 = 0.0;
        
        if (liveTime > 0 && analysisSuccess) {
            countRate630 = counts630 / liveTime;
            countRateError630 = error630 / liveTime;  // 计数率统计误差
            
            countRate740 = counts740 / liveTime;
            countRateError740 = error740 / liveTime;  // 计数率统计误差
        }
        
        // 进行死时间修正并计算总误差
        double correctedRate630 = 0.0, correctedError630 = 0.0;
        double correctedRate740 = 0.0, correctedError740 = 0.0;
        
        if (analysisSuccess) {
            // 计算630 keV峰的修正后计数率和误差
            calculateCorrectedRate(countRate630, countRateError630,
                                 deadTimePercent, deadTimeErrorPercent,
                                 correctedRate630, correctedError630);
            
            // 计算740 keV峰的修正后计数率和误差
            calculateCorrectedRate(countRate740, countRateError740,
                                 deadTimePercent, deadTimeErrorPercent,
                                 correctedRate740, correctedError740);
        }
        
        // 输出结果 - 修改为630和740 keV峰
        std::cout << std::left << std::setw(25) << displayFileName;
        ...
        if (analysisSuccess) {
            ...
            std::cout << std::right << std::setw(18) 
                      << std::fixed << std::setprecision(4) << countRate630 
                      << " ± " << std::setprecision(4) << countRateError630 << " ";
            
            // 630 keV峰：修正后计数率±误差
            std::cout << std::right << std::setw(18) 
                      << std::fixed << std::setprecision(4) << correctedRate630 
                      << " ± " << std::setprecision(4) << correctedError630 << " ";
            
            // 740 keV峰：原始计数率±误差
            std::cout << std::right << std::setw(18) 
                      << std::fixed << std::setprecision(4) << countRate740 
                      << " ± " << std::setprecision(4) << countRateError740 << " ";
            
            // 740 keV峰：修正后计数率±误差
            std::cout << std::right << std::setw(18) 
                      << std::fixed << std::setprecision(4) << correctedRate740 
                      << " ± " << std::setprecision(4) << correctedError740;
    ...
    std::cout << "Analyzed peaks: 630 keV (628-632 keV range) and 740 keV (738-742 keV range)" << std::endl;
