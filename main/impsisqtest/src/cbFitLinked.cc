// cbFitLinked.cc — 241Am alpha peak fits from CoMPASS RAW data
...
// Usage:
//   root -l -q 'src/cbFitLinked.cc(9)'              — run all fits
//   root -l -q 'src/cbFitLinked.cc(9, "cb")'        — single Crystal Ball only
//   root -l -q 'src/cbFitLinked.cc(9, "dg")'        — linked double Gaussian only
//   root -l -q 'src/cbFitLinked.cc(9, "dcb")'       — linked double Crystal Ball only
...
Bool_t LoadParticleFluence(Int_t nRunNumber, Double_t &dParticleFluence)
{
    ...
    // TString strInfoPath = TString::Format("data/A1442B_DT5780_compass/run_%d/run_%d_info.txt", nRunNumber, nRunNumber);
    // TString strInfoPath = TString::Format("data/A1442B_DT5780_compass/run_%d/run_%d_info.txt", nRunNumber, nRunNumber);
    TString strInfoPath = TString::Format("data/A1442B_V2745B_compass/run_%d/run_%d_info.txt", nRunNumber, nRunNumber);
...
void cbFitLinked(Int_t nRunNumber = 9,
                 ...
                 Bool_t bDraw = kTRUE)
{
    ...
    // TString strFilePath = TString::Format("data/A1442B_DT5780_compass/run_%d/RAW/DataR_CH0@DT5780N_1068_run_%d.root", nRunNumber, nRunNumber);
    // TString strFilePath = TString::Format("data/A1442B_DT5780_compass/run_%d/RAW/DataR_run_%d.root", nRunNumber, nRunNumber);
    TString strFilePath = TString::Format("data/A1442B_V2745B_compass/run_%d/RAW/DataR_run_%d.root", nRunNumber, nRunNumber);
    ...
    Long64_t nEntries = fTree->Draw(strDrawExpr, "Timestamp<2e15", "goff");
    ...
    if (bFitDG) {
        ...
        fDG->SetParLimits(2, dSigmaInit * 0.75, dSigmaInit * 1.25);
